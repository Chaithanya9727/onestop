...
import { MapPin, Globe, ExternalLink, Clock, Building2, ChevronRight, DollarSign } from "lucide-react";
...
const JobCard = ({ job }) => {
   ...
   const handleAction = (e, url) => {
      ...
      // If authenticated and it's an external link, the anchor tag handles it naturally.
      // If it's an internal navigation (passed as url string but empty/null here implies check), handle nav.
               ...
               <div className="flex items-center justify-between pt-5 border-t border-slate-100 dark:border-white/5 mt-auto">
                  ...
                  {job.source === 'OneStop' || !job.source || job.isInternal ? (
                     <Link
                        ...
                        className="text-slate-900 dark:text-white font-bold text-sm flex items-center gap-1 group-hover:translate-x-1 transition-transform group-hover:text-blue-600 dark:group-hover:text-blue-400"
                     >
