            ...
            <div className="lg:col-span-2 space-y-8">
                  ...
                  <div className="flex-1">
                     <div className="flex justify-between items-start">
                        <div>
                           ...
                           <p className="text-lg font-bold text-blue-600 mb-2 flex items-center gap-2">
                           ...
                           <div className="flex items-center gap-2 mb-4">
                              ...
                              <span className="font-bold text-slate-900 dark:text-white">{mentor.averageRating}</span>
                              <span className="text-slate-500 text-sm">({mentor.totalReviews} Reviews)</span>
                           </div>
                        </div>
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                     {mentor.mentorProfile?.services?.map((service, idx) => (
                        <div
                           ...
                           className={`cursor-pointer p-6 rounded-2xl border-2 transition-all ${selectedService === service
               ...
               <div>
                  <h3 className="text-xl font-black text-slate-800 dark:text-white mb-6 flex items-center gap-2">
                     <Star className="text-amber-500" fill="currentColor" /> Reviews <span className="text-slate-400 text-sm font-medium">({mentor.totalReviews})</span>
                  </h3>
                           ...
                           <div key={i} className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700">
                              <div className="flex items-center gap-3 mb-3">
                                 <div className="w-10 h-10 bg-slate-200 rounded-full overflow-hidden">
                                    {review.mentee?.avatar ? <img src={review.mentee.avatar} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-blue-500" />}
                                 </div>
                                 <div>
                                    <h4 className="font-bold text-slate-900 dark:text-white text-sm">{review.mentee?.name || "Anonymous"}</h4>
                                    <div className="flex items-center gap-1 text-amber-500 text-xs">
                                       {[...Array(5)].map((_, starI) => (
                                          <Star key={starI} size={10} fill={starI < review.rating ? "currentColor" : "none"} className={starI < review.rating ? "" : "text-slate-300"} />
                                       ))}
                                       <span className="text-slate-400 ml-2">{new Date(review.createdAt).toLocaleDateString()}</span>
                                    </div>
                                 </div>
                              </div>
                              <p className="text-slate-600 dark:text-slate-300 text-sm italic">"{review.review}"</p>
                           </div>
                        ))
                     ) : (
                        <div className="p-8 text-center bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-dashed border-slate-200 dark:border-slate-700 text-slate-500">
                     ...
                     <div className="space-y-6">
                           ...
                           <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
                              {getNextDays().map(date => (
                                 <button
                                    ...
                                    className={`flex-shrink-0 px-4 py-2 rounded-xl text-sm font-bold border transition-all ${selectedDate === date ? 'bg-slate-900 text-white border-slate-900' : 'bg-white border-slate-200 text-slate-600 hover:border-slate-400'
                                       }`}
                                 >
                                 ...
                                 {getSlotsForDay(selectedDate).length > 0 ? (
                                    getSlotsForDay(selectedDate).map(slot => (
                                       <button
                                          ...
                                          className={`px-2 py-2 rounded-lg text-xs font-bold border transition-all ${selectedTime === slot ? 'bg-blue-600 text-white border-blue-600' : 'bg-slate-50 border-slate-200 text-slate-600 hover:bg-slate-100'
                                             }`}
                                       >
                                          {slot}
                                       </button>
                        ...
                        <div className="pt-4 border-t border-slate-100 dark:border-slate-700">
                           ...
                           <button
                              ...
                              className={`w-full py-4 rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-xl ${selectedTime ? 'bg-blue-600 hover:bg-blue-500 text-white shadow-blue-500/30' : 'bg-slate-200 text-slate-400 cursor-not-allowed'
                                 }`}
                           >
