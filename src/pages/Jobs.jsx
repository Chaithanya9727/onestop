...
export default function Jobs() {
  ...
  const handleJobClick = (job) => {
    const isExternal = job.isExternal || (job.source && job.source !== 'OneStop');

    // 🔒 Gatekeeper for External Jobs: Must login to view/apply
  ...
  const { oneStopJobs, externalJobs } = useMemo(() => {
    const internal = [];
    const external = [];
    filteredJobs.forEach(job => {
      if (job.source === 'OneStop' || !job.source) {
        internal.push(job);
      } else {
        external.push(job);
      }
    });
    return { oneStopJobs: internal, externalJobs: external };
  }, [filteredJobs]);
  ...
  const JobCard = ({ job, index, handleJobClick, getRelativeTime }) => (
    <motion.div
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#0a0a0a] text-slate-900 dark:text-white pb-20 relative overflow-hidden transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 relative z-10 transition-colors">
        ...
        <div className="flex justify-center mb-8">
          <div className="bg-slate-100 dark:bg-white/5 p-1.5 rounded-2xl inline-flex gap-2 border border-slate-200 dark:border-white/5">
            <button
              onClick={() => setActiveTab("internal")}
              className={`px-6 py-3 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === "internal"
                ? "bg-white dark:bg-[#0f1014] text-blue-600 dark:text-blue-400 shadow-md transform scale-105"
                : "text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"
                }`}
            >
              <Briefcase size={16} /> OneStop Exclusives
            </button>
            <button
              onClick={() => setActiveTab("external")}
              className={`px-6 py-3 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === "external"
                ? "bg-white dark:bg-[#0f1014] text-purple-600 dark:text-purple-400 shadow-md transform scale-105"
                : "text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"
                }`}
            >
              <Globe size={16} /> External Marketplace
            </button>
          </div>
        </div>
        ...
        {filteredJobs.length === 0 ? (
          ...
          <div className="min-h-[400px]">
            {/* Section 1: OneStop Exclusives */}
            {activeTab === "internal" && oneStopJobs.length > 0 ? (
              <motion.div
              ...
              >
                <div className="flex items-center justify-between mb-8 border-b border-blue-500/10 pb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">OneStop Exclusives</h2>
                      <p className="text-xs font-black text-blue-600 dark:text-blue-400 uppercase tracking-[0.2em] mt-0.5">Direct Verified Applications</p>
                    </div>
                  </div>
                  <div className="text-xs font-black text-slate-400 uppercase tracking-widest bg-slate-100 dark:bg-white/5 px-4 py-2 rounded-full border border-slate-200 dark:border-white/5">
                    {oneStopJobs.length} Positions
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  <AnimatePresence mode="popLayout" initial={false}>
                    {oneStopJobs.map((job, index) => (
                      <JobCard key={job._id || job.id || index} job={job} index={index} handleJobClick={handleJobClick} getRelativeTime={getRelativeTime} />
                    ))}
                  </AnimatePresence>
                </div>
              </motion.div>
            ) : activeTab === "internal" && (
              <div className="text-center py-20 text-slate-500 dark:text-slate-400">No internal jobs found matching your criteria.</div>
            )}

            {/* Section 2: External Opportunities */}
            {activeTab === "external" && externalJobs.length > 0 ? (
              <motion.div
                initial={{ opacity: 0, x: 20 }}
                animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, x: -20 }}
                transition={{ duration: 0.3 }}
              >
                <div className="flex items-center justify-between mb-8 border-b border-purple-500/10 pb-4">
                  <div className="flex items-center gap-3">
                    <div className="p-3 bg-purple-600 rounded-2xl shadow-lg shadow-purple-600/20">
                      <Globe className="text-white" size={24} />
                    </div>
                    <div>
                      <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">External Marketplace</h2>
                      <p className="text-xs font-black text-purple-600 dark:text-purple-400 uppercase tracking-[0.2em] mt-0.5">Across the Global Web</p>
                    </div>
                  </div>
                  <div className="text-xs font-black text-slate-400 uppercase tracking-widest bg-slate-100 dark:bg-white/5 px-4 py-2 rounded-full border border-slate-200 dark:border-white/5">
              ...
              <div className="text-center py-20 text-slate-500 dark:text-slate-400">No external jobs found matching your criteria.</div>
            )}
          </div>
