...
export default function Resources() {
  ...
  const canManage = ["candidate", "admin", "superadmin"].includes(role?.toLowerCase());
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#0a0a0a] text-slate-900 dark:text-white pb-20 relative overflow-hidden transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-20">
        ...
        <div className="bg-white/80 dark:bg-[#0f1014]/80 backdrop-blur-xl rounded-[2rem] border border-slate-200 dark:border-white/5 p-2 mb-8 flex flex-col md:flex-row gap-4 justify-between items-center sticky top-4 z-40 shadow-xl shadow-slate-200/50 dark:shadow-2xl">
           ...
           {tabValue === 0 && user && canManage && (
              <div className="flex gap-3 w-full md:w-auto">
                 <button onClick={() => navigate(filter === 'my' ? '/resources' : '/resources?filter=my')} className={`px-5 py-3 rounded-xl font-bold text-sm border transition-all ${filter === 'my' ? 'bg-slate-900 text-white border-slate-900 dark:bg-white dark:border-white dark:text-black' : 'bg-transparent text-slate-600 dark:text-slate-400 border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/30 hover:text-slate-900 dark:hover:text-white'}`}>
                    My Uploads
                 </button>
                 <button onClick={() => setShowForm(!showForm)} className="flex items-center gap-2 px-5 py-3 bg-blue-600 text-white rounded-xl font-bold text-sm hover:bg-blue-500 transition-all shadow-lg shadow-blue-600/20 active:scale-95">
                    {showForm ? <X size={18} /> : 
                       <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                   ...
                   {list.map(r => (
                      <motion.div layout key={r._id} className="relative h-full">
                         <ResourceCard resource={r} user={user} role={role} onEdit={startEdit} onDelete={remove} />
                         {tabValue === 3 && (
           ...
           <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
              ...
              {loading ? (
                 ...
                 <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {videos.map((v) => (
                       <div key={v.id} onClick={() => setSelectedVideo(v)} className="group cursor-pointer bg-white dark:bg-[#0f1014] rounded-[1.5rem] border border-slate-200 dark:border-white/5 p-3 hover:border-blue-200 dark:hover:border-white/10 transition-all hover:-translate-y-1 shadow-sm hover:shadow-xl dark:shadow-none">
