...
import { Filter, ChevronDown, Check, Rocket, Code, Sword } from 'lucide-react';
...
const Hackathons = () => {
  ...
  const platforms = [
    { id: 'unstop', name: 'Unstop', color: '#1C4980' },
    { id: 'devpost', name: 'Devpost', color: '#003E54' },
    { id: 'hackerearth', name: 'HackerEarth', color: '#2C3454' },
  ];
  ...
  const filteredHackathons = useMemo(() => {
    return hackathons.filter(h => {
       const platformMatch = selectedPlatforms.length === 0 || 
           selectedPlatforms.some(p => h.site?.toLowerCase().includes(p));
       return platformMatch;
    });
  }, [hackathons, selectedPlatforms]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0a0a0a] text-slate-900 dark:text-white pb-20 relative overflow-hidden transition-colors duration-300">
      ...
      <div className="fixed inset-0 pointer-events-none">
          <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-pink-100 dark:bg-pink-600/10 rounded-full blur-[120px] transition-colors" />
      ...
      <div className="max-w-7xl mx-auto px-6 pt-12 relative z-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
           <div>
              <motion.div initial={{opacity: 0, y: -20}} animate={{opacity: 1, y: 0}} className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-pink-50 dark:bg-pink-500/10 border border-pink-200 dark:border-pink-500/20 text-pink-600 dark:text-pink-400 text-xs font-bold uppercase tracking-widest mb-4">
                 <Rocket size={14} /> Innovate & Build
              </motion.div>
              <motion.h1 initial={{opacity: 0, x: -20}} animate={{opacity: 1, x: 0}} className="text-4xl md:text-6xl font-black text-slate-900 dark:text-white tracking-tight leading-tight">
                 Global <span className="text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-purple-500 dark:from-pink-400 dark:to-purple-400">Hackathons</span>
              </motion.h1>
           </div>
           ...
           <div className="bg-white/80 dark:bg-white/5 backdrop-blur-md p-1.5 rounded-2xl border border-slate-200 dark:border-white/10 flex shadow-sm">
               ...
               <Link to="/hackathons" className="px-6 py-3 bg-white dark:bg-white text-black font-bold rounded-xl shadow-lg transform transition-all flex items-center gap-2 hover:scale-105 border border-slate-100 dark:border-transparent">
        ...
        <div className="bg-white dark:bg-[#0f1014] border border-slate-200 dark:border-white/10 rounded-3xl p-6 mb-12 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl shadow-slate-200/50 dark:shadow-2xl relative overflow-hidden transition-all">
           ...
           <div className="relative w-full md:w-72 z-20">
              <button 
                ...
                className="w-full bg-slate-50 dark:bg-black/40 text-slate-900 dark:text-white border border-slate-200 dark:border-white/10 px-5 py-4 rounded-xl flex items-center justify-between font-bold hover:border-pink-500/50 hover:shadow-lg transition-all"
              >
                 <span className="flex items-center gap-3">
                   <Filter size={20} className="text-pink-600 dark:text-pink-500" /> 
                   {selectedPlatforms.length > 0 ? <span className="text-pink-600 dark:text-pink-400">{selectedPlatforms.length} Selected</span> : 'Filter by Platform'}
                 </span>
                     ...
                     {platforms.map(platform => (
                        <div 
                          ...
                          className={`flex items-center gap-3 px-4 py-3 rounded-xl cursor-pointer transition-all ${
                             selectedPlatforms.includes(platform.id) ? 'bg-pink-600 text-white font-bold' : 'hover:bg-slate-100 dark:hover:bg-white/10 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
                          }`}
                        >
                           <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-colors ${
                              selectedPlatforms.includes(platform.id) ? 'border-white bg-white text-pink-600' : 'border-slate-300 dark:border-slate-600'
                           }`}>
        ...
        {!loading && filteredHackathons.length === 0 && (
          <div className="text-center py-32 bg-white dark:bg-[#0f1014] rounded-[3rem] border border-slate-200 dark:border-white/5 shadow-xl">
            <div className="inline-flex p-8 rounded-full bg-slate-50 dark:bg-white/5 mb-6 shadow-inner">
               <Rocket size={48} className="text-slate-400 dark:text-slate-600" />
            </div>
            <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">No hackathons found</h3>
