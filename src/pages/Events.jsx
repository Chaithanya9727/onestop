...
export default function Events() {
   ...
   const [timeFilter, setTimeFilter] = useState("all"); // 'all', 'live', 'upcoming', 'past'
   ...
   const load = async () => {
      try {
         setLoading(true);
         const statusParam = timeFilter === "all" ? "" : timeFilter;
         const catParam = category === "all" ? "" : category;
         const data = await get(`/events?search=${search}&page=${page}&limit=${pageSize}&status=${statusParam}&category=${catParam}`);
   ...
   return (
      <div className="min-h-screen bg-slate-50 dark:bg-[#0a0a0a] py-10 px-6 relative overflow-hidden transition-colors duration-300">
         ...
         <div className="max-w-7xl mx-auto space-y-8 relative z-10">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
               <div>
                  ...
                  <motion.h1 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-4xl font-black text-slate-900 dark:text-white flex items-center gap-3">
                     Explore <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-purple-600 dark:from-indigo-400 dark:to-purple-400">Global Events</span>
                  </motion.h1>
                  <p className="text-slate-600 dark:text-slate-400 font-medium mt-2 max-w-lg">Join world-class hackathons, workshops, and hiring challenges designed to accelerate your career.</p>
               </div>

               <div className="flex items-center gap-4">
                  {(isAdmin || isSuperAdmin) && (
                     <Link to="/events/create" className="flex items-center gap-2 px-6 py-3 bg-white dark:bg-white/10 hover:bg-slate-50 dark:hover:bg-white/20 text-slate-900 dark:text-white font-bold rounded-xl shadow-lg shadow-slate-200/50 dark:shadow-none border border-slate-200 dark:border-white/10 transition-all hover:scale-105 active:scale-95">
                        <Plus size={20} /> Create
                     </Link>
                  )}
                  <div className="flex bg-white dark:bg-[#1a1a1a] p-1.5 rounded-xl border border-slate-200 dark:border-white/5">
                     <button onClick={() => setViewMode("list")} className={`p-2 rounded-lg transition-colors ${viewMode === 'list' ? 'bg-slate-100 dark:bg-white/10 text-indigo-600 dark:text-white' : 'text-slate-400'}`}>
                        <ListIcon size={20} />
                     </button>
                     <button onClick={() => setViewMode("calendar")} className={`p-2 rounded-lg transition-colors ${viewMode === 'calendar' ? 'bg-slate-100 dark:bg-white/10 text-indigo-600 dark:text-white' : 'text-slate-400'}`}>
            ...
            {/* 🔍 Unified Filter Bar */}
            <div className="bg-white dark:bg-[#0f1014] p-2 rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-200 dark:border-white/5 flex flex-col xl:flex-row gap-4 items-center">
               ...
               <div className="relative w-full xl:w-80 group/search">
                  ...
                  <input
                     ...
                     className="w-full pl-11 pr-4 py-3 bg-slate-50 dark:bg-[#1a1a1a] border border-slate-200 dark:border-white/5 rounded-xl font-bold text-sm outline-none focus:border-indigo-500/50 text-slate-900 dark:text-white placeholder-slate-500 dark:placeholder-slate-600 transition-all"
                     placeholder="Search events..."
                  />
               </div>

               <div className="w-px h-8 bg-slate-200 dark:bg-white/10 hidden xl:block"></div>

               {/* Time Filters */}
               <div className="flex items-center gap-1 bg-slate-50 dark:bg-[#1a1a1a] p-1 rounded-xl border border-slate-200 dark:border-white/5 overflow-x-auto max-w-full">
                  {['all', 'live', 'upcoming', 'past'].map((tf) => (
                     <button
                        key={tf}
                        onClick={() => { setTimeFilter(tf); setPage(1); }}
                        className={`px-4 py-2 rounded-lg text-xs font-black uppercase tracking-wider whitespace-nowrap transition-all ${timeFilter === tf
                           ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20'
                           : 'text-slate-500 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-white/5'
                           }`}
                     >
                        {tf === 'all' ? 'All Time' : tf === 'live' ? 'Live Now' : tf}
                     </button>
                  ))}
               </div>
               ...
               {/* Category Filters */}
               <div className="flex items-center gap-1 overflow-x-auto w-full xl:w-auto noscrollbar">
                  {[
                     { id: 'all', label: 'All Events' },
                     { id: 'hackathon', label: 'Hackathons' },
                     { id: 'workshop', label: 'Workshops' },
                     { id: 'quiz', label: 'Quizzes' },
                     { id: 'job-challenge', label: 'Hiring' },
                  ].map((cat) => (
                     <button
                        key={cat.id}
                        onClick={() => { setCategory(cat.id); setPage(1); }}
                        className={`px-4 py-2 rounded-lg text-sm font-bold whitespace-nowrap transition-all ${category === cat.id
