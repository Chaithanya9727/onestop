      ...
      <div className="min-h-screen bg-slate-50 dark:bg-[#0a0a0a] pt-24 pb-20 px-4 relative overflow-hidden transition-colors duration-300">
         ...
         <div className="max-w-2xl mx-auto relative z-10">
            ...
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-10 text-center">
               ...
               <p className="text-slate-600 dark:text-slate-400 font-medium text-lg">Connect, share insights, and grow with the community.</p>
            </motion.div>
            ...
            <div className="bg-white dark:bg-[#0f1014] p-6 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-xl dark:shadow-none border border-slate-200 dark:border-white/5 mb-10 relative overflow-hidden group focus-within:border-blue-500/30 transition-colors">
               ...
               <div className="flex gap-4 relative z-10">
                  <div className="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-[#1a1a1a] overflow-hidden flex-shrink-0 border border-slate-200 dark:border-white/5">
                     {isAuthenticated && user?.avatar ? <img src={user.avatar} className="w-full h-full object-cover" alt="" /> : <div className="w-full h-full flex items-center justify-center bg-blue-600 text-white font-bold">{isAuthenticated ? user?.name?.charAt(0) : "?"}</div>}
                  </div>
               ...
               {loading ? (
                  ...
                  <AnimatePresence>
                     {posts.map(post => {
                        ...
                        const isAuthor = isAuthenticated && (post.author?._id === user?._id || user?.role === 'admin' || user?.role === 'superadmin');
                                       ...
                                       <div className="mt-6 pt-6 border-t border-slate-100 dark:border-white/5">
                                          <div className="flex gap-3 mb-6">
                                             <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-[#1a1a1a] flex-shrink-0 overflow-hidden">
                                                {isAuthenticated && user?.avatar ? <img src={user.avatar} className="w-full h-full object-cover" alt="" /> : <div className="w-full h-full flex items-center justify-center bg-blue-600 text-white text-xs font-bold">{isAuthenticated ? user?.name?.charAt(0) : '?'}</div>}
                                             </div>
                                          ...
                                          <div className="space-y-4 max-h-[300px] overflow-y-auto custom-scrollbar pr-2">
                                             {post.comments?.length > 0 ? post.comments.map((comment, index) => {
                                                const isCommentAuthor = isAuthenticated && (comment.user?._id === user?._id || user?.role === 'admin');
                                                return (
                                                   <div key={index} className="flex gap-3 text-sm group/comment">
                                                      <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-[#1a1a1a] flex-shrink-0 overflow-hidden">
                                                         {comment.user?.avatar ? <img src={comment.user.avatar} className="w-full h-full object-cover" alt="" /> : <div className="w-full h-full flex items-center justify-center bg-purple-600 text-white text-xs font-bold">{comment.user?.name?.charAt(0) || "?"}</div>}
                                                      </div>
