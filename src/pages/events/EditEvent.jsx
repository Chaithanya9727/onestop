...
export default function EditEvent() {
   ...
   const loadEvent = async () => {
      try {
         ...
         setForm({
            ...
            registrationDeadline: res.registrationDeadline ? res.registrationDeadline.slice(0, 16) : "",
            maxTeamSize: res.maxTeamSize || 1,
            prizes: res.prizes?.length ? res.prizes : [""],
            rules: res.rules?.length ? res.rules : [""],
            linkedJob: res.linkedJob?._id || res.linkedJob || "",
         });

         if (res.faqs) {
            setFaqs(res.faqs.map(f => ({ q: f.question, a: f.answer })));
         }

         if (res.coverImage?.url) {
            setCoverPreview(res.coverImage.url);
         }

      } catch (err) {
         showToast("Failed to load event data", "error");
         navigate("/manage/events"); // Back to Event Manager
      } finally {
         setLoading(false);
      }
   };
   ...
   const validateForm = () => {
      ...
      // Past date check removed for Edit (might be editing an old event)
      // But start < end check remains
      if (regDate > start) {
   ...
   const submit = async () => {
      ...
      try {
         ...
         await put(`/events/${id}`, fd, { headers: { "Content-Type": "multipart/form-data" } });
   ...
   if (loading) return <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-[#0a0a0a] transition-colors"><Loader className="animate-spin text-indigo-600 dark:text-indigo-400" size={40} /></div>;
   ...
   return (
      <div className="min-h-screen bg-slate-50 dark:bg-[#0a0a0a] text-slate-900 dark:text-white py-10 pb-24 relative overflow-hidden transition-colors duration-300">
         ...
         <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto px-6 relative z-10 transition-colors">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
               <div className="flex items-center gap-4">
                  ...
                  <div>
                     <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Edit Event</h1>
                     <p className="text-slate-500 dark:text-slate-400 font-medium">Update details for {form.title}</p>
                  </div>
               </div>
               <button onClick={submit} disabled={saving} className="px-8 py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-black rounded-2xl shadow-xl shadow-indigo-600/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3 disabled:opacity-70 disabled:cursor-not-allowed">
            ...
            <div className="grid gap-8">
               ...
               {/* Basic Info */}
               <div className="bg-white dark:bg-[#0f1014] p-8 md:p-10 rounded-[2.5rem] border border-slate-200 dark:border-white/5 shadow-xl shadow-slate-200/50 dark:shadow-none space-y-8 transition-colors">
                  ...
                  <p className="text-sm font-medium text-slate-500 dark:text-slate-400 -mt-4 ml-13">Modify the core information identifying your opportunity.</p>
               ...
               {/* Prizes & Rules */}
               <div className="grid md:grid-cols-2 gap-8">
                  <div className="bg-white dark:bg-[#0f1014] p-8 md:p-10 rounded-[2.5rem] border border-slate-200 dark:border-white/5 shadow-xl shadow-slate-200/50 dark:shadow-none h-full flex flex-col transition-colors">
                     <div className="flex justify-between items-center mb-8">
                        ...
                        <p className="text-sm font-medium text-slate-500 dark:text-slate-400 -mt-4 ml-13">Outline the incentives and recognition for participants.</p>
                  ...
                  <div className="bg-white dark:bg-[#0f1014] p-8 md:p-10 rounded-[2.5rem] border border-slate-200 dark:border-white/5 shadow-xl shadow-slate-200/50 dark:shadow-none h-full flex flex-col transition-colors">
                     <div className="flex justify-between items-center mb-8">
                        ...
                        <p className="text-sm font-medium text-slate-500 dark:text-slate-400 -mt-4 ml-13">Set the rules and regulations for the event.</p>
               ...
               {/* FAQs */}
               <div className="bg-white dark:bg-[#0f1014] p-8 md:p-10 rounded-[2.5rem] border border-slate-200 dark:border-white/5 shadow-xl shadow-slate-200/50 dark:shadow-none transition-colors">
                  <div className="flex justify-between items-center mb-8">
                     <h2 className="text-xl font-black flex items-center gap-3 text-slate-900 dark:text-white">
