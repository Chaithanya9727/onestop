...
import {
   Calendar, MapPin, Users, Trophy, BookOpen, HelpCircle, AlertTriangle, PlayCircle, CheckCircle, Clock, Share2, ArrowLeft, Loader, Star, Briefcase, Scroll, X
} from "lucide-react";
...
export default function EventDetails() {
   ...
   const [regData, setRegData] = useState({ teamName: "", responses: {} });

   const handleRegInputChange = (fieldId, label, val) => {
      setRegData(prev => ({
         ...prev,
         responses: { ...prev.responses, [fieldId]: { label, value: val } }
      }));
   };
   ...
   const handleRegister = async () => {
      try {
         setRegistering(true);
         const customResponses = Object.entries(regData.responses).map(([fieldId, obj]) => ({
            fieldId,
            label: obj.label,
            value: obj.value
         }));
         ...
         const updated = await get(`/events/${id}`);
         setEvent(updated);
      } catch (err) {
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-[#0a0a0a] text-slate-900 dark:text-white relative overflow-hidden transition-colors duration-300">
         ...
         <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-12 pt-8 pb-16">
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 pt-4">
                     {isCompleted ? (
                        ...
                        <div className="flex flex-col sm:flex-row w-full gap-4">
                           ...
                           {isRegistered && (
                              <button onClick={() => navigate(`/events/${id}/certificate`)} className="px-6 py-4 bg-yellow-50 dark:bg-yellow-500/10 border border-yellow-200 dark:border-yellow-500/20 text-yellow-700 dark:text-yellow-400 font-bold rounded-2xl hover:bg-yellow-100 dark:hover:bg-yellow-500/20 transition-all flex items-center justify-center gap-2 whitespace-nowrap shadow-lg shadow-yellow-200/50 dark:shadow-none">
            ...
            {event.rounds?.length > 0 && (
               <div className="mt-16 bg-white dark:bg-white/5 rounded-[2.5rem] p-10 border border-slate-200 dark:border-white/5 shadow-xl">
                  <h2 className="text-2xl font-black mb-10 flex items-center gap-4 text-slate-900 dark:text-white">
                     <span className="w-10 h-10 rounded-xl bg-orange-50 dark:bg-orange-500/20 text-orange-600 dark:text-orange-400 flex items-center justify-center"><Trophy size={20} /></span>
                     Opportunity Timeline & Pipeline
                  </h2>
                  <div className="grid md:grid-cols-4 gap-8 relative">
                     {/* Progress Bar Line */}
                     <div className="absolute top-1/2 left-0 right-0 h-1 bg-slate-100 dark:bg-white/5 -translate-y-1/2 hidden md:block z-0" />

                     {event.rounds.map((r, i) => {
                        const pStatus = event.participants?.find(p => p.userId === user?._id)?.roundStatus?.find(rs => rs.roundId === r.roundNumber);
                        const isCurrent = event.participants?.find(p => p.userId === user?._id)?.currentRound === r.roundNumber;
                        const isQualified = pStatus?.status === 'qualified';
                        const isDisqualified = pStatus?.status === 'disqualified';

                        return (
                           <div key={i} className="relative z-10 flex flex-col items-center text-center space-y-4 group">
                              <div className={`w-14 h-14 rounded-full flex items-center justify-center border-4 transition-all duration-500 ${isQualified ? 'bg-green-500 border-green-200 text-white' :
                                 isDisqualified ? 'bg-red-500 border-red-200 text-white' :
                                    isCurrent ? 'bg-blue-600 border-blue-200 text-white scale-110 shadow-lg shadow-blue-600/20' :
                                       'bg-white dark:bg-slate-800 border-slate-100 dark:border-white/10 text-slate-400'
                                 }`}>
                                 {isQualified ? <CheckCircle size={24} /> : <span className="font-black text-lg">{r.roundNumber}</span>}
                              </div>
                              <div className="space-y-1">
                                 <p className={`font-black uppercase tracking-widest text-xs ${isCurrent ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400'}`}>{r.type}</p>
                                 <h4 className={`font-bold text-base ${isCurrent ? 'text-slate-900 dark:text-white' : 'text-slate-500'}`}>{r.title}</h4>
                              </div>
                              {isDisqualified && <span className="px-3 py-1 bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 text-[10px] font-black rounded-lg uppercase">Disqualified</span>}
                              {isQualified && <span className="px-3 py-1 bg-green-50 dark:bg-green-500/10 text-green-600 dark:text-green-400 text-[10px] font-black rounded-lg uppercase">Qualified</span>}
                           </div>
                        );
                     })}
                  </div>
         ...
         <AnimatePresence>
            {showRegModal && (
               <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-6 backdrop-blur-md bg-black/40">
                  <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-white dark:bg-[#0f1014] w-full max-w-xl rounded-[2.5rem] border border-slate-200 dark:border-white/10 shadow-2xl overflow-hidden">
                     <div className="p-8 border-b border-slate-100 dark:border-white/5 flex justify-between items-center bg-slate-50/50 dark:bg-white/5">
                        <h2 className="text-2xl font-black text-slate-900 dark:text-white flex items-center gap-3">
                           <div className="w-10 h-10 rounded-xl bg-indigo-50 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 flex items-center justify-center"><Scroll size={20} /></div>
                           Complete Registration
                        </h2>
                        <button onClick={() => setShowRegModal(false)} className="p-2 hover:bg-slate-200 dark:hover:bg-white/10 rounded-xl transition-colors"><X size={20} /></button>
                     </div>

                     <div className="p-8 max-h-[60vh] overflow-y-auto space-y-8 custom-scrollbar">
                        {event.maxTeamSize > 1 && (
                           <div className="space-y-3">
                              <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Team Name</label>
                              <input type="text" value={regData.teamName} onChange={e => setRegData({ ...regData, teamName: e.target.value })} className="w-full p-4 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl font-bold" placeholder="Give your team a cool name..." />
                           </div>
                        )}

                        {event.customFields?.map((f, i) => (
                           <div key={i} className="space-y-3">
                              <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">{f.label} {f.required && <span className="text-red-500">*</span>}</label>
                              {f.type === 'url' ? (
                                 <input type="url" onChange={e => handleRegInputChange(f.id, f.label, e.target.value)} className="w-full p-4 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl font-medium" placeholder="https://..." />
                              ) : f.type === 'number' ? (
                                 <input type="number" onChange={e => handleRegInputChange(f.id, f.label, e.target.value)} className="w-full p-4 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl font-medium" />
                              ) : (
                                 <input type="text" onChange={e => handleRegInputChange(f.id, f.label, e.target.value)} className="w-full p-4 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl font-medium" placeholder={f.placeholder} />
                              )}
                           </div>
                        ))}
                     </div>
                     ...
                     <div className="p-8 bg-slate-50/50 dark:bg-white/5 border-t border-slate-100 dark:border-white/5 flex gap-4">
                        <button onClick={() => setShowRegModal(false)} className="flex-1 py-4 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-400 font-bold rounded-2xl hover:bg-slate-50 transition-all">Cancel</button>
                        <button onClick={handleRegister} disabled={registering} className="flex-[2] py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-black rounded-2xl shadow-xl shadow-indigo-600/20 transition-all flex items-center justify-center gap-2">
                           {registering ? <Loader className="animate-spin" size={20} /> : "Confirm Registration"}
                        </button>
                     </div>
                  </motion.div>
               </motion.div>
